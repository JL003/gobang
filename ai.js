...
function isWin(x,y,data) {
    let linkNum = 0;
        ...
        linkNum = 0;
        // 左边斜线
        for(let i = 0; i < 9 && area.x3 + i <= area.x2 && area.y3 + i <= area.y2; i++ ) {
            let diffX = area.x3 + i;
            let diffY = area.y3 + i;
        ...
        linkNum = 0;
        // 右边斜线
        for(let i = 0; i < 9 && area.x4 - i >= area.x1 && area.y4 + i <= area.y2; i++ ) {
            let diffX = area.x4 - i;
            let diffY = area.y4 + i;
            let diffObj = dataObj[diffX + '-' + diffY];
            if(diffObj['color'] === data['color']) {
...
function wall(x, y) {
    ...
    for(let key in dataObj) {
        if(dataObj[key]['color']) {
            ...
            let nowLine = getLine(lineX, lineY, dataObj[key]['color'], dataObj[key]['color'] === 'white' && 'black' || 'white', 4);
    ...
    if(whiteWarning && blackWarning && whiteWarning[0]['value'] < blackWarning[0]['value'] && blackWarning[0]['value'] >= 2) {
        planOne = false;
    }
    if(planOne) {
        ...
        if(getWhiteArr.length > 0) {
            ...
            if(position['value'] === 0) {
                ...
                if(position.value1 + position.null1 > position.value2 + position.null2 && position.null1 > 0) {
                    position = directionFn[position.key1](position.x, position.y ,position.value1);
                } else {
                    position = directionFn[position.key2](position.x, position.y ,position.value2);
                }
            }
